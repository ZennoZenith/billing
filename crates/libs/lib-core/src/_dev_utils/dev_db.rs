...
// NOTE: Must be same as env:SERVICE_DB_URL defined in .cargo/config.toml
const PG_DEV_APP_URL: &str =
    "postgres://example_user:dev_only_pwd@localhost/example_db";
...
pub async fn init_dev_db() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // -- Set demo1 pwd
    let demo1_user = UserBmc::first_by_user_id(&ctx, &mm, "demo1")
        .await?
        .unwrap();
    UserBmc::update_pwd(&ctx, &mm, &demo1_user.user_id, DEMO_PWD).await?;
